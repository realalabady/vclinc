...
export function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "appointments" | "telemedicine" | "patients"
  >("overview");
  ...
  const startSession = (id: number) => {
    setTelemedicineSessions((prev) =>
      prev.map((session) =>
        session.id === id
          ? { ...session, status: "in-progress" as const, duration: 0 }
          : session,
      ),
    );
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="border-b border-gray-200 bg-deep">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            <Link to={`/${locale}`}>
              <img
                src="/logos/logo_laite.jpg"
                alt="V Clinic"
                className="h-10"
              />
            </Link>
            <div className="hidden h-8 w-px bg-offwhite/20 sm:block" />
            <h1 className="hidden text-lg font-semibold text-offwhite sm:block">
              {t.title}
            </h1>
          </div>
          <div className="flex items-center gap-4">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex gap-2 overflow-x-auto rounded-xl bg-white p-2 shadow-sm">
          {(
            ["overview", "appointments", "telemedicine", "patients"] as const
          ).map((tab) => (
            <button
              key={tab}
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-3xl font-bold text-deep">
                  ...
                  <tbody>
                    {appointments.slice(0, 3).map((apt) => (
                      <tr key={apt.id} className="border-b border-gray-50">
                        <td className="py-3 font-medium text-deep">
                          {apt.patientName}
                        </td>
                        ...
                        <td className="py-3">
                          <span
                            className={`rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(apt.status)}`}
                          >
        ...
        {activeTab === "appointments" && (
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-6 text-xl font-semibold text-deep">
              {t.appointments}
            </h2>
            <div className="overflow-x-auto">
              <table className="w-full">
        ...
        {activeTab === "telemedicine" && (
          <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              {telemedicineSessions.filter((s) => s.status !== "completed")
                .length === 0 ? (
        ...
        {activeTab === "patients" && (
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="py-12 text-center text-charcoal/60">
              ...
              <p>
                {isArabic
                  ? "قريباً - إدارة بيانات المرضى"
                  : "Coming Soon - Patient Management"}
              </p>
            </div>
          </div>
        )}
